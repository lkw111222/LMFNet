...
class test_dataset:
    ...
    def load_data(self, max_attempts=10):
        attempts = 0

        while attempts < max_attempts:
            if self.index >= min(len(self.images), len(self.gts), len(self.depths)):
                print(f"[TestLoader] Reached end of dataset at index {self.index}, resetting to 0.")
                self.index = 0
            ...
            try:
                ...
                print(
                    f"[Error] Index {self.index} out of range (images: {len(self.images)}, gts: {len(self.gts)}, depths: {len(self.depths)})")
                self.index = 0
                attempts += 1
                continue
            ...
            try:
                ...
                print(f"[Warning] Failed to load image/depth at index {self.index}, skipping. Error: {e}")
                self.index += 1
                attempts += 1
                continue
            ...
            self.index += 1
            return image_tensor, gt, depth_tensor, name, np.array(image_for_post)

        # ⛔ 若达到最大尝试次数仍失败
        raise RuntimeError(f"[load_data] Failed to load valid data after {max_attempts} attempts.")
