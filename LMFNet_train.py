...
from models.PLFRNet import LMFNet
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
            ...
            if i % 100 == 0 or i == total_step or i == 1:
                print(
                    ...
                    format(datetime.now(), epoch, opt.epoch, i, total_step,
                           optimizer.state_dict()['param_groups'][0]['lr'], sal_loss.item(),sal_loss1.item()))
                logging.info(
                    ...
                    format(epoch, opt.epoch, i, total_step, optimizer.state_dict()['param_groups'][0]['lr'],
                           sal_loss.item(), sal_loss1.item(), memory_used))
                writer.add_scalar('Loss', loss.item(), global_step=step)
                grid_image = make_grid(images[0].clone().cpu(), 1, normalize=True)
                writer.add_image('RGB', grid_image, step)
                grid_image = make_grid(gts[0].clone().cpu(), 1, normalize=True)
        ...
        del images, gts, depth
        del s, s1, s2, s3
        del sal_loss1, sal_loss2, sal_loss3, sal_loss4, sal_loss
        del loss
